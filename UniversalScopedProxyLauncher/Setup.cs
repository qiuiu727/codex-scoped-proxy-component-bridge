...
internal static class Setup
{
    ...
    private static int Install()
    {
        ...
        try
        {
            ...
            CreateStartMenu(launcher, defaultPath, chinese); if (desktop) CreateShortcut(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), Product + ".lnk"), launcher, "--tray", defaultPath, Product);
            using (var key = Registry.CurrentUser.CreateSubKey(UninstallKey)) { key.SetValue("DisplayName", Product); key.SetValue("DisplayVersion", "1.1.0"); key.SetValue("Publisher", "qiuiu727"); key.SetValue("InstallLocation", defaultPath); key.SetValue("DisplayIcon", launcher + ",0"); key.SetValue("UninstallString", "\"" + Path.Combine(defaultPath, "Uninstall.exe") + "\" --uninstall"); key.SetValue("NoModify", 1); key.SetValue("NoRepair", 1); }
    ...
    private static void CreateStartMenu(string launcher, string directory, bool chinese) { string group = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Programs), Product); Directory.CreateDirectory(group); CreateShortcut(Path.Combine(group, chinese ? "代理程序托盘.lnk" : "Proxy launcher tray.lnk"), launcher, "--tray", directory, Product); CreateShortcut(Path.Combine(group, chinese ? "启动代理程序.lnk" : "Launch applications through proxy.lnk"), launcher, "--launch", directory, Product); CreateShortcut(Path.Combine(group, chinese ? "配置代理程序.lnk" : "Configure proxy launcher.lnk"), launcher, "--configure", directory, Product); }
